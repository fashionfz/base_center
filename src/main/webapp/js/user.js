function getUser(){
    ...
    $('#dg').datagrid({
	    ...
	    columns:[[ 
	        ...
	        {field:'password',title:'密码',width:150,formatter:function(value,row){
				return "******";
			}},
            ...
            {field:'opter',title:'操作',width:200,align:'center',formatter:function(value,row,index){
				return "<button class='positive' onclick='modifyUser("+index+");'>修改</button>&nbsp;&nbsp;"+
                "<button class='positive' onclick='deleteUser(\""+row.id+"\",\""+row.name+"\");'>删除</button>&nbsp;&nbsp;"+
                "<button class='positive' onclick='anth(\""+row.id+"\",\""+row.name+"\");'>授权</button>"
			}}
	    ]]
	});
}
